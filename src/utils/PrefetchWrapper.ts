...
class PrefetchWrapperSW {
    ...
    _batchID: NodeJS.Timeout | undefined = undefined;

    /** @type {Set.<String>} */
    _cachedAssets = new Set();
     ...
     * @private
     */
    _addToPipeline(assets: { all: Set<string>, map: Map<string, Array<string>> }) {
    ...
    signalResume() {
        ...
        const tryBatch = (selfID: NodeJS.Timeout) => {
    ...
    /** @private */
    _fetchBatch(selfID: NodeJS.Timeout) {
        const promises = [];
 ...
 *      `urgent` 急用；<br/>
 */
function extractAssetUrl(sceneText: string, {
...
function initServiceWorkerPrefetchWrapper() {
    ...
    if (contScene && (contScene !== 'start.txt' || runtime.SentenceID)) prefetcher.suggestNextScene(getUrl(contScene, 'scene'), runtime.SentenceID);
}
