 ...
 */
export const preForward = (continueAfterSettling = false, commitOptions: IStageCommitOptions = {}) => {
  ...
  if (hasUnsettledNonHoldPerform) {
    ...
    WebGAL.gameplay.performController.settleNonHoldPerforms(!continueAfterSettling, commitOptions);
 ...
 */
export const continueSentence = (commitOptions: IStageCommitOptions = {}) => {
  const GUIState = webgalStore.getState().GUI;
  if (GUIState.showTitle) {
    return;
  }

  if (!preForward(true, commitOptions)) {
    return;
  }
...
export const nextSentence = (commitOptions: IStageCommitOptions = {}) => {
  ...
  if (!preForward(false, commitOptions)) {
    return;
  }
